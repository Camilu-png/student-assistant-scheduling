...
def penalty_free(solution, data):
    penalty = 0
    assigned_days = [
        day for day in range(data.num_days) if np.any(solution.X[day, :, :])
    ]
    for student in range(data.num_students):
        if len(assigned_days) == 0:
            ...
            if all(
                np.sum(data.students[:, day, student]) == 0 for day in assigned_days
            ):
                penalty += 1
    return penalty
...
def count_students(solution, data):
    ...
    for student in range(data.num_students):
        ...
        for day in range(data.num_days):
            for slot in range(data.num_slots):
                if data.students[slot, day, student] == 1 and solution.is_assigned(
                    day, slot
                ):
                    can_attend = True
                    break
